...
class TRTEngine:
    ...
    def __init_bindings(self) -> None:
        dynamic = False
        Tensor = namedtuple('Tensor', ('name', 'dtype', 'shape', 'cpu'))
        inp_info = []
        out_info = []
        ...
        for i, name in enumerate(self.input_names):
            ...
            inp_info.append(Tensor(name, dtype, shape, cpu))
        ...
        for i, name in enumerate(self.output_names):
            ...
            out_info.append(Tensor(name, dtype, shape, cpu))
    ...
    def __warm_up(self) -> None:
        if self.is_dynamic:
            ...
            return
        for _ in range(10):
            inputs = [i.cpu for i in self.inp_info]
    ...
    def __call__(self, *inputs) -> Tuple[ndarray, ndarray, ndarray]:
        ...
        for i in range(self.num_inputs):
            if self.is_dynamic:
                ...
                self.inp_info[i] = self.inp_info[i]._replace(
                    cpu=np.empty(contiguous_inputs[i].shape, contiguous_inputs[i].dtype)
                )

            self.inp_info[i].cpu[:] = contiguous_inputs[i]
